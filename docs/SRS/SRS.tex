...
\\
$\mathbf{\^{y}}_(n \times m)$ & Unitless & Matrix representing the probability values of the pixels of the predicted label of the input image
\\
$\mathbf{z}_(n \times m)$ & Unitless & Matrix representing the combination of pixels from the input image and weights and biases, used in the Predicted Label Matrix \italic{\mathbf{\^y}}
\\
...
~\newline
$\^y}$ represents the predicted label value
~\newline
...
% #6 Notes
{
The values of $y$ and $\^y$ can be scalar or matrices, but they must be the of the same size (see \dref{A1}).
}
% #7 Source
{
  \url{https://towardsdatascience.com/logistic-regression-from-scratch-69db4f587e17}
...
SI Units&Unitless\\
\hline
Equation&$ \mathbf{\^y} = \sigma(z) = 1 \div (1 + (e^-\mathbf{z})) \text{, where} z = \mathbf{w}^T \cdot \mathbf{x} + b$ \\
\hline
Description &
...
&$b$ is the bias
\\
Note that the predicted value $\mathbf{\^y}$ from \dref{TM2} will be determined from the sigmoid function.
\hline
...
SI Units&Unitless\\
\hline
Equation&$ \nabla_\mathbf{w} \[ \mathbf{y}_i \cdot \log(\mathbf{\^y}_i)\ + (1 - \mathbf{y}_i) \cdot \log(1 - \mathbf{\^y}_i)\] = \mathbf{x}_n \cdot (\mathbf{y}_n - \mathbf{\^y}_n)   $\\
\hline
Description &
Here we derive the Log Loss Function with respect to $\mathbf{w}$  using chain rule from \dref{TM3}, with $\mathbf{\^y} $
...
SI Units&Unitless\\
\hline
Equation&$ \nabla_b \[ \mathbf{y}_i \cdot \log(\mathbf{\^y}_i)\ + (1 - \mathbf{y}_i) \cdot \log(1 - \mathbf{\^y}_i)\] = \mathbf{y}_n - \mathbf{\^y}_n   $\\
\hline
Description &
Here we derive the Log Loss Function with respect to $b$ using chain rule from \dref{TM3}, with $\mathbf{\^y}$ 
defined using the Sigmoid Function (see \dref{GD1}), so that we can minimize the Log Loss Function to train the model (see \dref{IM1}).
...
\begin{center}
  \begin{equation}
    \[ -\mathbf{y} \cdot \log(\mathbf{\^y})\]' = -\mathbf{y} \div \mathbf{\^y}, \\
    \text{and,} \\
    \[-(1 - \mathbf{y}) \cdot \log(1 - \mathbf{\^y}) \]' = (1 - \mathbf{y}) \div (1 - \mathbf{\^y}), \\
    \text{which combines to,}\\
    (-\mathbf{y} \div \mathbf{\^y}) + ((1 - \mathbf{y}) \div (1 - \mathbf{\^y})) = (\mathbf{\^y} - \mathbf{y}) \div (\mathbf{\^y} \cdot (1 - \mathbf{\^y}))
  \end{equation}
\end{center}
Then we take the derivative of $\mathbf{\^y} = \sigma(\mathbf(z))$ using the following identities:
\begin{center}
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
  Input&$\mathbf{x}_n$, $\mathbf{y}_n$, $\mathbf{\^y}_n = 1 \div (1 + (e^(\mathbf{w}^T \cdot \mathbf{x} + b))), \alpha \text{from \dref{DD1}}, \lambda \text{from \dref{DD2}}, N$, which is the number of training images\\
  ...
  \begin{itemize}
    ...
    \begin{itemize}
        \item 1.1. $d\mathbf{w}^(t) = \mathbf{x}_n \cdot (\mathbf{y}_n - \mathbf{\^y}_n) - (\lambda \div N) \cdot \mathbf{w}^(t)^2 $
        \item 1.2. $db^(t) = \mathbf{y}_n - \mathbf{\^y}_n$
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
  Output & $\mathbf{\^y}$, and the string representation of the corresponding predicted label
  \hline
