...
\begin{figure}[h!]
  ...
  To maintain consistency within the software the images will be normalized to some set of $n\timesm$ pixels such that
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  each of the 26 label models to determine the most likely label as the Sigmoid Output that is in value to 1. In addition 
  the confidence level in the prediction is represented by the output of the sigmoid function for the predicted label.
  \\
  \hline
  Sources & \citet{Turin2020, SharmaLogReg2022} \\
  \hline
