...
\\
$\boldface{z}_(n \times m)$ & Unitless & Matrix representing the combination of pixels from the input image and weights and biases, used in the Predicted Label Matrix \italic{\boldface{\^y}}
\\
$x_(i,j)$ & Unitless & An entry in the input matrix \boldface{x} (at i, j) which represents the level of intensity of the pixel
\\
