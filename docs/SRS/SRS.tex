...
as described by \iref{IM1}, will only contain labels of capital letter characters.
\item[A\refstepcounter{assumpnum}\theassumpnum \label{A3}:] The dataset used for training the software model
as described by \iref{IM1}, will contain an even spread of every label that can be identified by the porgram.
...
\hline
\rowcolor[gray]{0.9}
Number& TM\refstepcounter{defnum}\thedefnum \label{TM1}\\
\hline
...
the confidence level of the predicted value, to satisfy s{GS3}.
\\
\hline
...
\hline
\rowcolor[gray]{0.9}
Number& TM\refstepcounter{defnum}\thedefnum \label{TM2}\\
\hline
...
\hline
  Source & \url{https://towardsdatascience.com/logistic-regression-from-scratch-69db4f587e17} \\
  \hline
  Ref.\ By & \aref{A1}, \dref{GD1}, \dref{GD2}, \dref{GD3}, \iref{IM1}\\
  \hline
\end{tabular}
\end{minipage}\\
...
\hline
\rowcolor[gray]{0.9}
Number& TM\refstepcounter{defnum}\thedefnum \label{TM3}\\
\hline
Label &\bf Chain Rule \\
\hline
% Units&$MLt^{-3}T^0$\\
...
\hline
  Source & \url{https://towardsdatascience.com/logistic-regression-from-scratch-69db4f587e17} \\
  \hline
  Ref.\ By & \dref{GD2}, \dref{GD3}, \iref{IM1}, \iref{IM2}\\
  \hline
\end{tabular}
\end{minipage}\\
...
defined using the Sigmoid Function (see \dref{GD1}), so that we can minimize the Log Loss Function to train the model (see \iref{IM1}).\\
\hline
...
The goals s{GS2} and s{GS3} are solved by \iref{IM2}. \iref{IM1} is used 
to train the model so that \iref{IM2} can be used reliably and efficiently.
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
 ...
 \end{itemize}
  ...
  \hline
  Ref.\ By & \aref{A2}, \aref{A3}, \iref{IM2}, \rref{R3}\\
  \hline
\end{tabular}
\end{minipage}\\
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  \hline
  Description &
  Using the optimized weights and bias from \iref{IM1} and combining that with an unknown input $\mathbf{x}$ in
...
\hline        
	& \tref{TM1}& \tref{TM2}& \tref{TM3}& \dref{GD1}& \dref{GD2} & \dref{GD3}& \ddref{DD1} & \ddref{DD2}& \iref{IM1}& \iref{IM2}\\
\hline
