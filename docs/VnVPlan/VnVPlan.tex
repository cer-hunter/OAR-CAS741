...
To ensure code consistency, linters and coverage tools will be implemented as described in \ref{Tools}. Furthermore,
...
\begin{enumerate}
  ...
  Input: A set of non-corrupt (valid) PNG, JPG, BMP and PDF image files depicting valid label characters (see Figure \ref{Fig_A}).
...
\begin{enumerate}
  ...
  Input: A set of non-corrupt (valid) image files depicting valid label characters (see Figure \ref{Fig_A}) of the following sizes:
  \begin{itemize}
...
\item{\textbf{T\refstepcounter{testnum}\thetestnum \label{T_inputAngle}}: Test input for images with different orientations\\}
  ...
  Input: A set of non-corrupt (valid) image files depicting valid classifiable label characters (see Figure \ref{Fig_angleA}) at the following orientations:
  \begin{itemize}
    \item{$0\degree$}
    \item{$\pm 15\degree$}
    \item{$\pm 30\degree$}
...
\begin{enumerate}
  ...
  Input: A set of non-corrupt (valid) image files depicting all 26 valid label characters(Figure \ref{Fig_A}), and a blank image depicting no character (\ref{Fig_blank}).
...
\begin{enumerate}
  ...
  \begin{itemize}
    \item{Training Set Image (Figure \ref{Fig_trainA}): Confidence $> 90\%$}
    \item{Test Set Image (Figure \ref{Fig_trainA}): Confidence $> 50\%$}
    \item{Multiple Character Image (Figure \ref{Fig_ABC}): Confidence $< 50\%$}
    \item{Blank Image (Figure \ref{Fig_blank}): Confidence $< 10\%$ (Corresponding to the "No Label Detected" Output)}
  \end{itemize}
...
\begin{enumerate}
  ...
  Note: CircleCI tests are built and executed in a Docker Image, a container that enables the program to be portable,
