...
\begin{enumerate}
  ...
  Input: A subset of non-corrupt (valid) PNG, JPG, and BMP image files depicting valid label characters (see Figure \ref{Fig_A}).
  The exact list of files is as follows:
  \begin{itemize}
    \item{A.png}
    \item{A.jpg}
    \item{A.bmp}
  ...
  Output: The program should give a valid output as assigned by the oracle (the author). These outputs corresponding to the above
  inputs should be:
  \begin{itemize}
    \item{Some classification output (regardless of if it's correct)}
    \item{Some classification output (regardless of if it's correct)}
    \item{Some classification output (regardless of if it's correct)}
  \end{itemize}
...
\begin{enumerate}
  ...
  \item{\textbf{T\refstepcounter{testnum}\thetestnum \label{T_inputImage}}: Test input for Invalid formats\\}
            
  Control: Automatic
            
  Initial State: OAR loaded and idle.
            
  Input: A subset of directory paths pointing to a PDF image file, an empty file path, and no file path.
  The exact list of files is as follows:
  \begin{itemize}
    \item{A.png}
    \item{A.jpg}
...
\begin{enumerate}
  ...
  Input: A set of non-corrupt (valid) image files depicting valid label characters in black and white (Figure \ref{Fig_BWA}), 
  ...
  Input: A set of non-corrupt (valid) image files depicting valid label characters (Figure \ref{Fig_A}) of the following sizes:
  \begin{itemize}
...
\item{\textbf{T\refstepcounter{testnum}\thetestnum \label{T_inputAngle}}: Test input for images with different orientations\\}
            
  Control: Automatic
            
  Initial State: OAR loaded and idle.
            
  Input: A set of non-corrupt (valid) image files depicting valid classifiable label characters generated by paint.net (see Figure \ref{Fig_angleA}) at the following orientations:
  \begin{itemize}
    \item{$0\degree$}
    \item{$\pm 1\degree$}
    \item{$\pm 2\degree$}
  ...
  Test Case Derivation: These images are not compatible with the software according to A4, however detecting that orientation is
...
To satisfy R4 and R5 (and by extension R3) from the SRS \citep{SRS}, any input image should be properly classified with a confidence level and related to the 
...
\begin{enumerate}
  ...
  Input: A set of non-corrupt (valid) image files depicting all 26 valid label characters (Figure \ref{Fig_A}).
...
\begin{enumerate}
  ...
  \item{\textbf{T\refstepcounter{testnum}\thetestnum \label{T_outputLabel}}: Test Output of Degenerate Cases\\}
            
  Control: Automatic
            
  Initial State: OAR loaded and idle.
            
  Input: A set of non-corrupt (valid) image files depicting degenerate cases such as multiple letters (Figure \ref{abc_image}), a picture, and a blank image depicting no character (Figure \ref{Fig_blank}).
...
\begin{enumerate}
  ...
  viewing and writing code. The standard version of Flake 8 will be used, no config file will be needed.
...
\begin{table}[h!]
  ...
  \tref{T_inputAngle}           &X&X& &X& & &X& & \\ \hline
...
\begin{figure}[h!]
  \begin{center}
    \includegraphics[width=0.6\textwidth]{angleA}
